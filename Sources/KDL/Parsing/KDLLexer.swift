...
public class KDLLexer {
    ...
    public func nextToken() throws -> KDLToken {
        ...
        case "#":
            ...
            if matches("#true") {
                ...
                detectVersionIfNeeded(.v2)
                return KDLToken(
                    type: .boolean(false), location: startLocation, leadingTrivia: currentTrivia)
            } else if matches("#null") {
                detectVersionIfNeeded(.v2)
                return KDLToken(type: .null, location: startLocation, leadingTrivia: currentTrivia)
            } else if matches("#inf") {
                return KDLToken(
                    type: .decimal(Double.infinity), location: startLocation, leadingTrivia: currentTrivia)
            } else if matches("#-inf") {
                return KDLToken(
                    type: .decimal(-Double.infinity), location: startLocation, leadingTrivia: currentTrivia)
            } else if matches("#nan") {
                return KDLToken(
                    type: .decimal(Double.nan), location: startLocation, leadingTrivia: currentTrivia)
            }
    ...
    private func lexMultiLineString() throws -> KDLToken {
        ...
        while !isAtEnd() {
            if peek() == "\"" && peekNext() == "\"" && peekAt(2) == "\"" {
                ...
                return KDLToken(
                    type: .string(processedValue), location: startLocation, leadingTrivia: currentTrivia)
            } else if peek() == "\\" {
                advance()
                if !isAtEnd() {
                    let escaped = try lexEscapeSequence()
    ...
    private func lexRawString() throws -> KDLToken {
        ...
        while !isAtEnd() {
            if matches(closing) {
                return KDLToken(
                    type: .rawString(value), location: startLocation, leadingTrivia: currentTrivia)
            }
            value.append(advance())
        }
    ...
    private func lexNumber() throws -> KDLToken {
        ...
        if peek() == "#" {
            if matches("#inf") {
                ...
                return KDLToken(
                    type: .decimal(sign * Double.infinity), location: startLocation,
        ...
        if peek() == "0" && (peekNext() == "x" || peekNext() == "o" || peekNext() == "b") {
            ...
            if hasDecimalPart {
                ...
                return KDLToken(
                    type: .decimal(value), location: startLocation, leadingTrivia: currentTrivia)
            }
        }
    ...
    private func lexIdentifier() throws -> KDLToken {
        ...
        return KDLToken(
            type: .identifier(identifier), location: startLocation, leadingTrivia: currentTrivia)
    }
    ...
    private func lexTypeAnnotation() throws -> KDLToken {
        ...
        return KDLToken(
            type: .typeAnnotation(typeName), location: startLocation, leadingTrivia: currentTrivia)
    }
    ...
    private func isIdentifierStart(_ char: Character) -> Bool {
        return char.isLetter || char == "_" || char == "-" || char.isCurrencySymbol
            || char.unicodeScalars.first!.value > 127
    }
